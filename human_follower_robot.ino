...
if((distance > 1 ) && (distance > 75)){            //check wheather the ultrasonic sensor's value stays between 1 to 15.
  ...
  // Turn RIGHT toward the object detected on the right
  ...
  // Turn LEFT toward the object detected on the left
  Motor1.setSpeed(150);  // Left motor front
  Motor1.run(BACKWARD);
  Motor2.setSpeed(150);
  Motor2.run(BACKWARD);
  Motor3.setSpeed(150);  // Right motor back
  Motor3.run(FORWARD);
  Motor4.setSpeed(150);
  Motor4.run(FORWARD);
...
else if(distance > 75) {                          //If the condition is 'true' then the statement below will execute:
